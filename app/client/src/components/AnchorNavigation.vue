<template>
	<div class="fixed right-5 overflow-y-scroll h-4/5">
		<ul class="toc hidden lg:flex border-l border-stone-700 pl-2 grow flex-col gap-y-3 py-4">
			<li class="text-sm" v-for="header of headers" :key="header.id" :ref="header.id">
				<a v-bind:style="anchorStyles(header.level)" :href="`#${header.id}`">{{ header.text }}</a>
			</li>
		</ul>
	</div>
...
export default {
	...
	methods: {
		...
		anchorStyles(level) {
			if (parseInt(level) > 2) {
				const increment = 0.25 * level
				const marginLeft = (0.75 + increment).toString()
				return {
					marginTop: '0.5rem',
